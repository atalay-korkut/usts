...
def generate_all_points(image, n_points_per_side=16):
    H, W, _ = image.shape
...
with block:
        ...
        with gr.Row().style(equal_height=True):
            with gr.Column():
                ...
                def store_video_and_extract_frame(video_file):
                    ...
                    original_image.value = frame.copy()  # Store original frame for resetting
                    return frame, [], None
                video_upload.upload(
                ...
                segment_all_button = gr.Button('Segment Entire Frame')
    ...
    def get_point(img, sel_pix, point_type, evt: gr.SelectData):
        ...
        return img
    segment_all_button.click(
            generate_all_points,
            inputs=[input_image],
            outputs=[input_image, selected_points, last_mask]
        )
    input_image.select(
        get_point,
    ...
    def undo_points(orig_img, sel_pix):
        if not sel_pix:
            return orig_img, [], None  # If no points, just return the original image

        sel_pix.pop()  # Remove the last point from the list
        temp = orig_img.copy()  # Copy the original image to start fresh

    # Redraw all remaining points
        for point, label in sel_pix:
            cv2.drawMarker(temp, point, colors[label], markerType=markers[label], markerSize=20, thickness=5)
        return temp, sel_pix, None
